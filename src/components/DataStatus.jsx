...
export function DataStatus({ language }) {
  ...
  return <section className="global-section data-health" id="data-status" aria-labelledby="data-health-title">
    <div className="global-heading"><h2 id="data-health-title">{t.title}</h2><span>V0.5</span></div>
    <p>{t.intro}</p>
    ...
    <div className="health-grid">{sourceStatus(now).map(source => <article key={source.id} className="health-card"><a href={source.sourceUrl} target="_blank" rel="noreferrer">{source.id} ↗</a><h3>{t.sourceTitles?.[source.id] || source.title}</h3><small>{source.frequency === 'monthly' ? t.monthly : t.annual}</small><p className={source.old ? 'health-old' : ''}>{source.old ? t.old : t.available}</p><dl><dt>{t.latest}</dt><dd>{source.latest || '—'}</dd><dt>{t.updated}</dt><dd>{source.updated}</dd><dt>{t.retrieved}</dt><dd>{source.retrieved}</dd><dt>{t.missing}</dt><dd>{source.missing.toLocaleString(language)} / {source.total.toLocaleString(language)}</dd></dl></article>)}</div>
