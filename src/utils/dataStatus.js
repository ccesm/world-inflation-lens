...
export function observationStatus(points, frequency, now = new Date()) {
  ...
  const age = latest === null ? Infinity : frequency.startsWith('annual') ? now.getUTCFullYear() - Number(latest.slice(0, 4)) : frequency === 'monthly' ? monthNumber(now.toISOString().slice(0, 7)) - monthNumber(latest) : (now.getTime() - Date.parse(latest)) / 86400000
  const threshold = frequency === 'daily' || frequency.startsWith('weekly') ? 14 : 3
