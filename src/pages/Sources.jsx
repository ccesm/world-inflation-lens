...
import { researchLinks, releaseCalendars } from '../data/researchLinks.js'
...
export function Sources({ t, language }) {
  const labels = experience[language], w = workspaceCopy[language]
  return <><PageIntro {...t.sources} description={labels.sourcesDescription} /><section className="content-section"><div className="section-heading"><div><Eyebrow>{t.sources.sectionEyebrow}</Eyebrow><h2>{t.sources.sectionTitle}</h2></div><p>{labels.sourcesNote}</p></div>
    ...
    <section className="workspace workspace-help-links"><h2>{w.sources}</h2><div className="workspace-library">{researchLinks.map(link => <article className="workspace-card" key={link.url}><span className={`source-status ${link.status === 'integrated' ? 'integrated' : ''}`}>{link.status === 'integrated' ? labels.sourceStatus : (language === 'zh' ? '官方资料 · 尚未接入此数据集' : 'Official reference · dataset not integrated')}</span><h3><a href={link.url} target="_blank" rel="noreferrer">{link[language]} ↗</a></h3><p>{link.note[language]}</p></article>)}</div><h2>{w.calendars}</h2><p>{w.calendarNote}</p><div className="workspace-actions">{releaseCalendars.map(link => <a key={link.url} href={link.url} target="_blank" rel="noreferrer">{link[language]} ↗</a>)}</div></section>
