...
const colors = { headline: '#c46b46', food: '#409989', energy: '#8f87cf', rates: '#4499b5', oil: '#be993e' }
...
export function Drivers({ language }) {
  ...
  const topicStart = topic === 'food' ? driverSeries.food.points[0].date : driverStart
  const valid = /^\d{4}-(0[1-9]|1[0-2])$/.test(from) && /^\d{4}-(0[1-9]|1[0-2])$/.test(to) && from >= topicStart && to <= driverEnd && from < to
  ...
  const changeTopic = next => {
    setTopic(next)
    if (next !== 'food' && from < driverStart) setFrom(driverStart)
  }
  ...
  return <><PageIntro eyebrow={t.eyebrow} title={t.title} description={t.description} /><section className="content-section global-section drivers-section">
    ...
    <section className="global-panel driver-workbench" aria-label={t.dates}>
      ...
      <div className="driver-date-controls"><label>{t.from}<input aria-label={t.from} type="month" min={topicStart} max={driverEnd} value={from} onInput={event => setFrom(event.currentTarget.value)} onChange={event => setFrom(event.target.value)} /></label><label>{t.to}<input aria-label={t.to} type="month" min={topicStart} max={driverEnd} value={to} onInput={event => setTo(event.currentTarget.value)} onChange={event => setTo(event.target.value)} /></label><div className="range-control">{[5, 10, 50, null].map(count => <button key={count || 'all'} onClick={() => chooseRange(count)}>{count ? `${count}Y` : t.all}</button>)}</div></div>
      {!valid ? <p role="alert" className="driver-date-error">{t.dateError}</p> : <>
        ...
        <DriverChart series={rateSeries} dates={dates} selected={selected} onSelect={setMonth} title={topic === 'rates' ? t.rateAxis : t.yoy} unit="%" language={language} labels={t} episode={episode} />
      ...
      <div className="driver-source-list">{series.map(s => <p key={s.id}><a href={s.metadata.sourceUrl} target="_blank" rel="noreferrer">{s.name} · {s.id} ↗</a><span>{s.metadata.publisher}</span><span>{t.covered}: {driverSeries[s.key].points[0].date}–{driverSeries[s.key].points.at(-1).date} · {t.updated}: {s.metadata.sourceUpdatedAt} · {t.retrieved}: {s.metadata.retrievedAt}</span></p>)}</div>
    </section>
  </section></>
}
